...
1. 摄影区域的原片
    先读取照片在应用 EXIF 方向后的显示像素尺寸，记为 `W × H`。在成品上方或左侧以完全相同的 `W × H` 尺寸忠实展示这张照片。不得缩放、裁切、重画、替换、扩展、修饰、滤镜化或改变其可见像素。
    
2. 抽象面板的信息来源
    分析照片中的主体关系、大小比例、左右与上下位置、方向、曲线、水平轴、垂直轴、间距、重复、遮挡、前后层级、明暗层级、色彩角色和留白，再将这些关系重构为与原图相邻的抽象视觉记忆。整个抽象面板（包括背景、抽象母题和标题）必须与原图同为 `W × H` 像素。
...
抽象面板必须与原照片的显示像素尺寸完全相同。设原图为 `W × H` 像素，则完整抽象面板也必须为 `W × H` 像素，不得使用近似比例、百分比分配或视觉估算替代像素级相等。

如果图像生成器不能直接输出精确的 `W × H`，使用方向相同且宽高比最接近的可用画布，并为母题与标题保留充足安全边距。生成后只对抽象面板做等比缩放，并用与面板背景一致的均匀象牙色留白补齐至精确的 `W × H`。绝对不要缩放或裁切原照片，也不得拉伸或裁切抽象面板内容。

严格按原图方向选择拼接方式：

- 当 `W > H`（横向构图）时，将原照片放在上方，将抽象面板放在下方。最终成品尺寸必须为 `W × 2H`。
- 当 `W <= H`（纵向或正方形构图）时，将原照片放在左方，将抽象面板放在右方。最终成品尺寸必须为 `2W × H`。正方形照片归入这一规则。

两个区域各占最终成品的一半，并保持一比一的等尺寸关系。照片与面板之间采用无间隙、干净、直接、无阴影的平面衔接。不得使用撕纸边缘、相框、投影、立体卡片、胶带、拼贴阴影或样机效果。

完成后读取文件元数据验证：抽象面板必须是 `W × H`；横向原图的成品必须是 `W × 2H`；纵向或正方形原图的成品必须是 `2W × H`。验证失败时不得交付。
...
抽象母题放置在面板中央区域、中下部或由原照片关系支持的非对称位置。以 [assets/examples](../assets/examples) 中的作品作为尺度校准：母题应呈现为具有画面主导力的中大型构成，而不是漂浮在大片空白中的小图标。

以下百分比按母题全部抽象标记的整体外接范围计算，不包括标题和副标题：

- 对于位于原图下方的横向面板，母题通常占面板宽度的 58%–82%，高度占面板高度的 40%–58%。

- 对于位于原图右方的纵向或正方形面板，母题通常占面板宽度的 70%–88%，高度占面板高度的 50%–72%。

- 母题与面板边缘通常保留至少 7%–12% 的安全距离，并与标题保持清晰间隔。

- 母题外接范围可以较大，但实际着墨面积仍应克制。通过内部空隙、断续、疏朗叠合和不规则边缘，保留约 65%–80% 的可见象牙色背景；不要通过把整个母题缩小来制造留白。


根据主体类型允许调整：

- 地平线、海岸、道路、城市天际线或横向人群应优先使用范围上限，横向展开到面板宽度的 72%–88%，同时保持相对较低的高度。

- 高层建筑、树木、浪峰、斜向道路或其他纵向与对角结构可扩展到面板高度的 58%–75%，但不得触碰边缘或挤压标题。

- 只有当原照片的主体本身非常紧凑、孤立时，母题才可缩小到约 50%–65% 的面板宽度和 35%–50% 的面板高度；不得默认使用这一较小尺度。

- 紧凑有机群体应保持聚集，但整个群体仍需具有足够的视觉分量，不得退化为通用小图标。


母题内部的大小关系、方向、间距、遮挡、重复、重心和非对称必须作为一个整体保留。优先匹配参考图片呈现的中大型尺度与视觉分量；百分比仅用于防止母题过小或溢出，不得机械套用。
...
只输出一张完成的摄影与抽象面板拼接作品。使用无损 PNG 交付，以避免再次压缩原照片区域。交付前必须完成第五节规定的像素尺寸验证。
